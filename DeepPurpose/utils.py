...
from DeepPurpose.chemutils import MAX_NB, atom_features, bond_features, get_mol
...
def prauc_curve(y_pred, y_label, figure_file, method_name):
    ...
    plt.plot(
        ...
        lw=2,
        label=method_name
        + " (area = %0.2f)" % average_precision_score(y_label, y_pred),
    )
    fontsize = 14
...
def smiles2erg(s):
    try:
        ...
        print(
            "rdkit cannot find this SMILES for ErG: " + s + "convert to all 0 features"
        )
...
def target2quasi(s):
    try:
        ...
        print(
            "Quasi-seq fingerprint not working for smiles: "
            + s
...
def target2paac(s):
    try:
        ...
        print(
            "PesudoAAC fingerprint not working for smiles: "
            + s
            + " convert to 0 vectors"
        )
...
def target2ct(s):
    try:
        ...
        print(
            "Conjoint Triad fingerprint not working for smiles: "
            + s
            + " convert to 0 vectors"
        )
...
def encode_protein(
    ...
    if target_encoding == "AAC":
        print(
            "-- Encoding AAC takes time. Time Reference: 24s for ~100 sequences in a CPU.\
        ...
        print(
            "-- Encoding PseudoAAC takes time. Time Reference: 462s for ~100 sequences in a CPU.\
				 Calculate your time by the unique target sequence #, instead of the entire dataset."
        )
...
def data_process_repurpose_virtual_screening(
    X_repurpose, target, drug_encoding, target_encoding, mode
):
    if mode == "repurposing":
...
class data_process_loader(data.Dataset):
    def __init__(self, list_IDs, labels, df, **config):
        ...
        elif self.config["drug_encoding"] in [
            "DGL_GIN_AttrMasking",
            "DGL_GIN_ContextPred",
        ]:
            from dgllife.utils import (
...
def generate_config(
    ...
    if drug_encoding == "Morgan":
        ...
        ] = mlp_hidden_dims_drug  # MLP classifier dim 1
    elif drug_encoding == "ErG":
    ...
    if target_encoding == "AAC":
        ...
        ] = mlp_hidden_dims_target  # MLP classifier dim 1
    elif target_encoding == "PseudoAAC":
...
def download_pretrained_model(model_name, save_dir="./save_folder"):
    ...
    downloaded_path = os.path.join(
        pretrained_dir, name2zipfilename[model_name.lower()] + ".zip"
    )
