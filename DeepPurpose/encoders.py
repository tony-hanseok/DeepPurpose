...
class CNN_RNN(nn.Sequential):
    def __init__(self, encoding, **config):
        ...
        if encoding == "drug":
            ...
            self.conv = self.conv.double()
            n_size_d = self._get_conv_output(
                (63, 100)
            )  # auto get the seq_len of CNN output
            ...
            self.fc1 = nn.Linear(
                config["rnn_drug_hid_dim"] * direction * n_size_d,
        ...
        if encoding == "protein":
            ...
            self.rnn = self.rnn.double()
            self.fc1 = nn.Linear(
                config["rnn_target_hid_dim"] * direction * n_size_p,
    ...
    def forward(self, v):
        ...
        if self.encoding == "protein":
            if self.config["rnn_Use_GRU_LSTM_target"] == "LSTM":
                ...
                h0 = torch.randn(
                    self.config["rnn_target_n_layers"] * direction,
                    batch_size,
            ...
            if self.config["rnn_Use_GRU_LSTM_drug"] == "LSTM":
                ...
                h0 = torch.randn(
                    self.config["rnn_drug_n_layers"] * direction,
                    batch_size,
...
class MPNN(nn.Sequential):
    ...
    def forward(self, feature):
        ...
        output = [
            torch.mean(atom_hiddens.narrow(0, sts, leng), 0)
            for sts, leng in N_atoms_scope
        ]
...
class DGL_GCN(nn.Module):
    ...
    def __init__(
        self, in_feats, hidden_feats=None, activation=None, predictor_dim=None
    ):
...
class DGL_AttentiveFP(nn.Module):
    ...
    def __init__(
        ...
        self.readout = AttentiveFPReadout(
            feat_size=graph_feat_size, num_timesteps=num_timesteps
        )
