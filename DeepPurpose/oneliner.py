...
def repurpose(
    ...
    if agg == "mean":
        ...
        if convert_y:
            ...
            y_pred = (
                np.max(y_preds_models, axis=0) + np.mean(y_preds_models, axis=0)
            ) / 2
...
def virtual_screening(
    ...
    if train_drug is None:
        ...
        for idx, model_name in enumerate(pretrained_model_names):
            ...
            y_pred = models.virtual_screening(
                X_repurpose,
                target,
                model,
                drug_names,
    ...
    with open(os.path.join(result_folder_path, "logits_VS_mean_max.pkl"), "wb") as f:
        pickle.dump(
            (np.min(y_preds_models, axis=0) + np.mean(y_preds_models, axis=0)) / 2,
            f,
